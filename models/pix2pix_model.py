...
class Pix2PixModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='resnet_9blocks', dataset_mode='aligned')  # batch
        if is_train:
            parser.set_defaults(pool_size=0, gan_mode='vanilla')
            parser.add_argument('--lambda_L1', type=float, default=100.0, help='weight for L1 loss')
            parser.add_argument('--lambda_MS', type=float, default=1.0, help='weight for MS_SSIM loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_L1', 'G_GAN_l', 'D_real_g', 'D_fake_g', 'MS_SSIM', 'G_GAN_g', 'D_real_l', 'D_fake_l']  # , 'VGG',,,
        ...
        if self.isTrain:
            ...
            self.MS_SSIM = ssim.MS_SSIM(win_size=7, win_sigma=1.5, data_range=2, size_average=True, channel=1).to(self.device)  # 原本win_size是11
            # self.vgg = VGG16().cuda()
    ...
    def backward_D(self):
        ...
        pred_fake_g = self.netD_g(fake_AB.detach())
        self.loss_D_fake_l = self.criterionGAN(pred_fake_l, False)
        self.loss_D_fake_g = self.criterionGAN(pred_fake_g, False)
        # Real
        real_AB = self.fake_AB_pool.query(torch.cat((self.real_A, self.real_B), 1))
        pred_real_l = self.netD_l(real_AB.detach())  # 70*70
        pred_real_g = self.netD_g(real_AB.detach())  # 128*128
        self.loss_D_real_l = self.criterionGAN(pred_real_l, True)
        self.loss_D_real_g = self.criterionGAN(pred_real_g, True)
        # combine loss and calculate gradients
        self.loss_D = (self.loss_D_fake_l + self.loss_D_real_l) * 0.5  # 修改 + self.loss_D_fake_g + self.loss_D_real_g
    ...
    def backward_G(self):
        ...
        #     [int(self.real_B.size()[0]), 3, int(self.real_B.size()[2]), int(self.real_B.size()[3])]))[0]
        # self.loss_VGG = self.criterionL1(self.VGG_fake, self.VGG_real) * self.opt.lambda_vgg
        # SSIM loss
        ...
        self.loss_G = self.loss_G_GAN_l + self.loss_G_L1 + self.loss_G_GAN_g + self.loss_MS_SSIM  #  + self.loss_VGG
    ...
    #     return OrderedDict([('G_GAN_l', self.loss_G_GAN_l.item()),
    #                         ('G_GAN_g', self.loss_G_GAN_g.item()),
    #                         ('G_L1', self.loss_G_L1.item()),
    #                         # ('G_VGG', self.VGG_loss.data[0]),
    #                         ('D_real_l', self.loss_D_real_l.item()),
    #                         ('D_fake_l', self.loss_D_fake_l.item()),
    #                         ('D_real_g', self.loss_D_real_g.item()),
    #                         ('D_fake_g', self.loss_D_fake_g.item()),
